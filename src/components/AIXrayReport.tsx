  ...
  return (
    <section className="rounded-lg border border-primary/20 bg-card p-4 shadow-sm sm:p-5">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="font-heading text-xl font-bold text-foreground">🩻 AI X-Ray Report Generator</h2>
          <p className="text-sm text-muted-foreground">Balaji Ortho Care Center · Patient fee ₹50 · Doctor free</p>
        </div>
        <div className="rounded-md bg-primary/10 px-3 py-2 text-sm font-semibold text-primary">Gemini AI</div>
      </div>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <label className="space-y-1 text-sm font-medium">
          Patient Name
          <input className="w-full rounded-md border bg-background px-3 py-2 text-sm" value={name} onChange={(e) => setName(e.target.value)} placeholder="Patient name" />
        </label>
        <label className="space-y-1 text-sm font-medium">
          Age
          <input className="w-full rounded-md border bg-background px-3 py-2 text-sm" value={age} onChange={(e) => setAge(e.target.value)} placeholder="Age" inputMode="numeric" />
        </label>
        <label className="space-y-1 text-sm font-medium">
          Gender
        ...
        <label className="space-y-1 text-sm font-medium">
          Body Region
          <select className="w-full rounded-md border bg-background px-3 py-2 text-sm" value={body} onChange={(e) => setBody(e.target.value)}>
            {bodyParts.map((part) => <option key={part}>{part}</option>)}
          </select>
        </label>
        <label className="space-y-1 text-sm font-medium">
          Projection
          <select className="w-full rounded-md border bg-background px-3 py-2 text-sm" value={view} onChange={(e) => setView(e.target.value)}>
            {projections.map((projection) => <option key={projection}>{projection}</option>)}
          </select>
        </label>
        <label className="space-y-1 text-sm font-medium">
          X-Ray Image
          <input className="w-full rounded-md border bg-background px-3 py-2 text-sm" type="file" accept="image/*" onChange={(e) => handleFile(e.target.files?.[0] || null)} />
        </label>
      </div>

      <label className="mt-3 block space-y-1 text-sm font-medium">
        Clinical History
        <textarea className="min-h-20 w-full rounded-md border bg-background px-3 py-2 text-sm" value={clinical} onChange={(e) => setClinical(e.target.value)} placeholder="Pain, trauma history, swelling, follow-up etc." />
      </label>
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row">
        <button className="rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-60" onClick={analyze} disabled={!b64 || loading}>
          {loading ? "Analyzing..." : "Generate AI Report"}
        </button>
        {report && <button className="rounded-md border px-4 py-2 text-sm font-semibold" onClick={printReport}>Print Report</button>}
      </div>
      ...
      {report && (
        <article className="mt-5 space-y-4 rounded-lg border bg-background p-4 print:border-0 print:p-0">
          <header className="border-b pb-3">
            ...
            <p className="text-xs text-muted-foreground">AI X-Ray Report · {new Date().toLocaleDateString("en-IN")}</p>
          </header>
          <div className="grid gap-3 text-sm sm:grid-cols-2">
            <p><b>Patient:</b> {name || "Unknown"}</p>
            <p><b>Age/Gender:</b> {age || "—"} / {gender}</p>
            <p><b>Study:</b> {report.studyType || "—"}</p>
            <p><b>Urgency:</b> {report.urgency || "Routine"}</p>
            <p><b>Body Part:</b> {report.bodyPartDetected || body}</p>
            <p><b>Projection:</b> {view}</p>
          </div>
          <section>
          ...
          <section>
            ...
            <div className="space-y-1 text-sm">
              ...
              {report.findings?.bones && <p><b>Bones:</b> {report.findings.bones}</p>}
              {report.findings?.softTissues && <p><b>Soft Tissues:</b> {report.findings.softTissues}</p>}
              {report.findings?.specificFindings && <p><b>Specific Findings:</b> {report.findings.specificFindings}</p>}
              {report.findings?.extraFindings && <p><b>Extra Findings:</b> {report.findings.extraFindings}</p>}
            </div>
          </section>
          ...
          <section className="grid gap-3 text-sm sm:grid-cols-2">
            <div><h4 className="font-semibold text-primary">Differential Diagnosis</h4><p>{report.differentialDiagnosis || "None"}</p></div>
            <div><h4 className="font-semibold text-primary">Recommendations</h4><p>{report.recommendations || "—"}</p></div>
          </section>
